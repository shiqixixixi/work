...
wbdwdj= openpyxl.load_workbook('商品.xlsx')
...
old_excel = pd.read_excel('商品.xlsx', sheet_name='商品', 
                          usecols=['编号', '名称', '所属小类', '所属大类', '商品类型'],
                          dtype={'编号': str})

ckj_excel = pd.read_excel('商品.xlsx', sheet_name='商品')
...
for row in sorted(ref_price_rows.keys()):
    ...
    for col_num in sorted(ref_price_rows[row].keys()):
        ...
        result += f"{col_name}(参考价: {ref_price}.00 预估成本: 0)、" # 将每个单元格的数据后面添加一个'、'符号
